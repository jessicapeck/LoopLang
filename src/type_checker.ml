...
(* establish the types of the typing environments *)
type var_env = (var, t) Hashtbl.t
type inference_env = (var, t option ref) Hashtbl.t
...
(* type inference helper function to safely update the inference environment *)
let update_inference_env i_env name expected_type =
    match Hashtbl.find_opt i_env name with
    | Some(t_ref) -> (
        match !t_ref with
        | None -> 
...
let rec check_arg_types env i_env param_types args =
    let correct_arg_types = ref true in
    List.iter2(fun param_type arg ->
        let _, arg_type = check_argument env i_env param_type arg in
...
and get_func_types env i_env f args =
    ...
    let rec find_correct_type = function
        ...
        | t :: ts -> (
            ...
            if num_args = num_params then
                ...
                if check_arg_types env i_env copy_of_param_types args then (mangled_f, List.map unwrap_type copy_of_param_types, unwrap_type copy_of_return_type)
...
and get_var_type env i_env expected_t_opt v = 
    match Hashtbl.find_opt env v with
        ...
        | None -> (
            ...
            | Some(expected_t) -> unwrap_type (update_inference_env i_env v expected_t) (* return type inferred from inference environment *)
            | None -> (
                (* check if previously inferred *)
                match Hashtbl.find_opt i_env v with
                | Some(t_ref) -> (
                    match !t_ref with
                    | Some(t) -> unwrap_type t
                    | None ->
...
and check_expr env i_env expected_t_opt = function
    | Int(n) -> (Int(n), TInt)
    | Bool(b) -> (Bool(b), TBool)
    | ExprVar(v) -> 
        let t = get_var_type env i_env expected_t_opt v in
        (ExprVar(v), t)
    | ExprFuncCall(f, args) -> 
        let mangled_f, param_types, return_type = get_func_types env i_env f args in
        let args_ast = List.map2 (fun param_type arg ->
            let arg_ast, _ = check_argument env i_env param_type arg in
            arg_ast
        ) param_types args in
...
and check_stitch_seq env i_env expected_t = function
    | StitchSeq(seq) ->
        let seq_ast = List.map (fun item ->
            let item_ast, t_item = check_stitch_seq_item env i_env item in (* this (and parser) enforces TStitchSeqItem*)
            item_ast
        ) seq in
...
and check_argument env i_env expected_t = function
    ...
    | ArgExpr(e) -> 
        let e_ast, t = check_expr env i_env (Some expected_t) e in
        (ArgExpr(e_ast), t)
    | ArgStitchSeq(seq) -> 
...
let check_row_list_item env i_env = function
    ...
    | RowExpr(row_expr) -> 
        let row_expr_ast, t = check_row_expr env i_env row_expr in
        (RowExpr(row_expr_ast), t)

let check_definition env i_env = function
    | DefVar(v1, v2) -> (
        match Hashtbl.find_opt env v2 with
        | Some(t) -> Hashtbl.add env v1 t; (DefVar(v1, v2), env)
        | None -> (
            match Hashtbl.find_opt i_env v2 with
            | Some(t_ref) -> Hashtbl.add i_env v1 t_ref; (DefVar(v1, v2), env)
    ...
    | DefExpr(v, e) ->
        let e_ast, t = check_expr env i_env None e in
        Hashtbl.add env v t;
        (DefExpr(v, e_ast), env)
    | DefStitchSeq(v, seq) -> 
        let seq_ast, t = check_stitch_seq env i_env TStitchSeq seq in
...
let check_return_expr env i_env = function
    | ReturnVar(v) -> 
        let t = get_var_type env i_env None v in
        (ReturnVar(v), t)
...
let rec check_statement env i_env = function
    | CommentStmt(c) -> (CommentStmt(c), env, [])
    | LetDef(def) ->
        let def_ast, new_env = check_definition env i_env def in
        (LetDef(def_ast), new_env, [])
    | Row(row) -> 
        let row_ast, _ = check_row_lit env i_env row in
        (Row(row_ast), env, []) 
    | RowList(row_expr) ->
        let row_expr_ast, _ = check_row_expr env i_env row_expr in
        (RowList(row_expr_ast), env, [])
    | Return(ret_expr) ->
        let ret_expr_ast, t = check_return_expr env i_env ret_expr in
        (Return(ret_expr_ast), env, [t])
    | If(cond, then_branch, else_branch) ->
        let cond_ast, t_cond = check_expr env i_env (Some TBool) cond in
        if t_cond = TBool then
            ...
            let (then_branch_ast, env_after_then, ret_exprs_in_then) = 
                List.fold_left (fun (stmt_ast_acc, env_acc, ret_exprs_acc) stmt ->
                    let (stmt_ast, new_env, new_ret_exprs) = check_statement env_acc i_env stmt in
...
let check_pattern_item env = function
    | FuncDef(f, params, body) -> 
        (* initialise parameter types as unresolved *)
        let i_env : inference_env = Hashtbl.create 10 in
        List.iter (fun param -> Hashtbl.add i_env param (ref None)) params;

        let rec analyse_body local_env return_types body body_ast_acc =
            match body with
            | [] -> body_ast_acc, return_types
            | stmt :: stmts ->
                let stmt_ast, new_local_env, new_return_types = check_statement local_env i_env stmt
        ...
        (* get parameter types from inference environment *)
        let param_types = List.map(fun param ->
            match Hashtbl.find_opt i_env param with
            | Some(t_ref) -> (
                match !t_ref with
                | Some(t) -> t
                | None ->
            ...
            | None -> raise (InternalTypeError ("parameter '" ^ param ^ "' in function '" ^ f ^ "' not found in inference environment"))
        ) params in
    ...
    | Stmt(stmt) -> 
        let dummy_i_env : inference_env = Hashtbl.create 0 in
        let stmt_ast, new_env, _ = check_statement env dummy_i_env stmt in
